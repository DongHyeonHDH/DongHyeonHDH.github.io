...
#### 로컬에서 커밋 히스토리가 진행되는 순서
1. 로컬 리포지토리에 커밋 히스토리가 기록된다.
2. 커밋 하나당 커밋 메시지가 생기게 된다. 
3. 커밋 내역을 확인하고 이전 버전으로 돌아간다.
...
**참고**
github 들어갔을 때 설명하는 markdown은 잘 정리되어 있고 흥미를 돋구는 내용으로 작성할 것 <br>
  
>표시 : 인용구라는 의미  


**자주 일어날 수 있는 문제 사항**
git 자체는 뭐가 최신인지 이전 것인지를 구분하지 못한다(ambigious) →  사용자가 정해줄
필요가 있다.
...
<br>
[출처: 모찌vlog](https://mozzi-devlog.tistory.com/34)    
...
2. 상대경로법  
    - 현재 디렉토리(비교 대상)를 기준으로 작성된 경로를 의미
    -  /. : 내가있는곳  
    -  /.. : 내가 있는 곳 상위 디렉터리  
...
API의 일종으로 Java가 제공하는, Java프로그램 내에서 Database로 접근할 수 있도록 해주는 표준 인터페이스를 
JDBC라고 한다.
